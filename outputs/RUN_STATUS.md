...
Updated 2026-07-29 (12:50). W4 run 2 complete (`19185959`: L4/L5 rescored + L6–L8 ablation);
W7 complete; W8 assembled with resolving figure hashes. Scoring parity rescored after a parser
fix (`19189218`) — 2 of 6 models pass on clean data, OLMo 0.9904 and Qwen-7B 0.9565, so the
open-vs-closed bridge holds. **Roadmap #7 (reasoning dose–response) is blocked on API
billing** and has been re-scoped: N_SAMPLES 20 → 2, templates 4 → 2, provider Batch APIs, est.
~$580 per `59_dose_cost_model.py`. Batch runner built and validated offline but **not
submitted** — awaiting a top-up.
...
| W4 — Prompt curriculum | complete | **19130876** (run 1, preserved in `_w4_prefix_fewshot_bug/`); **19185959** (run 2: L4/L5 rescore + L6–L8 ablation) | v3; 6 models × 8 levels. Run 1's few-shot block asked about the target item's agent under the example's story on `human_verbatim`, invalidating L4/L5 on 1 of 7 templates; bootstrap unit corrected from template × group cell to scenario group. L4 label polarity verified non-inverted and now gated. Verdict level-specific: 3/6 clear the bar at L5; gains at the top level come from accidental falling, but **principle-alone (L8) is intent-led in 2/6** (Mistral Δattempted +0.241 with accidental flat) and beats the full stack in 3/6 |
| Scoring parity (open logprob-EV vs sampled) | complete, **rescored** | **19189218** | Re-run after `_parse_rating` stopped clamping out-of-range answers and `_finish` stopped imputing the midpoint. OLMo 0.9904, Qwen2.5-7B 0.9565 PASS; sub-7B models 0.45–0.93 do not. Pre-fix artifacts in `analysis/_scoring_parity_clamped/` |
