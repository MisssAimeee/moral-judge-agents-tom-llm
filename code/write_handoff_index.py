...
RULES = [
    ...
    (r"^surface_baseline\.csv$",
     ...
     "including span-matched rows (full / belief_last / action_last text prefixes).",
     "code/experiments/21_surface_baseline.py", CUR),
    (r"^.*_probe_belief_last_src.*\.csv$",
     "Belief-clause probe accuracy restricted to one source corpus (YS2008/YS2009/YS2011).",
     "code/02_probe.py", CUR),
    (r"^.*_probe_action_last_src.*\.csv$",
     "Action-clause probe accuracy restricted to one source corpus (YS2008/YS2009/YS2011).",
     "code/02_probe.py", CUR),
    (r"^gap_over_surface_span_matched\.csv$",
     "Peak probe − TF-IDF gap using span-matched surface baselines (clause cuts vs full story).",
     "code/experiments/44_span_matched_gaps.py", CUR),
    (r"^gap_over_surface_within_model_paired\.csv$",
     "Within-model paired intent-gap vs outcome-gap under span-matched baselines, with sign tests.",
     "code/experiments/33_gap_dissociation_figure.py", CUR),
    (r"^gap_over_surface_dissociation_span_matched\.png$",
     "Span-matched gap-over-surface figure (clause-position TF-IDF fit on truncated text).",
     "code/experiments/44_span_matched_gaps.py", CUR),
    (r"^C2_SOURCE_SPLIT_BELIEF_LAST\.md$",
     "C2 source-split (YS2008 vs YS2009) at belief_last against span-matched outcome TF-IDF, "
     "with a manual annotation audit (5 verbatim YS2009 stories, spans marked). Downgraded "
     "to supporting evidence: setup-hazard content TF-IDF can't generalize, not outcome "
     "anticipation or an annotation error.",
     "code/experiments/44_span_matched_gaps.py", CUR),
    (r"^TOM_VS_CONTRAST\.md$",
     "J1: ToM-benchmark accuracy vs moral contrast with base/instruct confound controls "
     "(instruct-only, OLS covariates, within-family deltas). All-20 r is confound demo only.",
     "code/experiments/42_tom_vs_contrast.py", CUR),
    (r"^tom_vs_contrast\.csv$",
     "Per-model ToM accuracy and moral contrast used by J1.",
     "code/experiments/42_tom_vs_contrast.py", CUR),
    (r"^tom_vs_contrast\.png$",
     "Scatter of ToM accuracy vs moral contrast (J1).",
     "code/experiments/42_tom_vs_contrast.py", CUR),
    (r"^tom_accuracy_by_model\.csv$",
     "Per-model BigToM / ToMi accuracy from the ToM gate and full runs.",
     "code/experiments/36_tom_benchmarks.py", CUR),
    (r"^mixed_effects_2x2\.csv$",
     "J3: per-model 2×2 cell means, interaction coefficient, and cell-order vs Young 2007 human pattern.",
     "code/experiments/39_mixed_effects_2x2.py", CUR),
    (r"^MIXED_EFFECTS_2x2\.md$",
     "J3 writeup: interaction coefficients with cell means and matches_human/inverted/tied column.",
     "code/experiments/39_mixed_effects_2x2.py", CUR),
    (r"^mixed_effects_interaction\.png$",
     "J3 interaction visualization across models.",
     "code/experiments/39_mixed_effects_2x2.py", CUR),
    (r"^FLOOR_DERIVATION\.md$",
     "J4: derived engagement and effect floors; dual-use policy (engagement/anchors vs correlations).",
     "code/experiments/40_derive_floors.py", CUR),
    (r"^floor_derivation\.csv$",
     "Numeric floor cutoffs derived for engagement filtering.",
     "code/experiments/40_derive_floors.py", CUR),
    (r"^ITEM_LEVEL_DISSOCIATION\.md$",
     "J2 item-level dissociation null with minimum meaningful slope and triad of independent nulls.",
     "code/experiments/41_item_level_dissociation.py", CUR),
    (r"^item_level_dissociation\.csv$",
     "Item-level probe vs behavioral contrast dissociation data.",
     "code/experiments/41_item_level_dissociation.py", CUR),
    (r"^item_level_groups\.csv$",
     "Grouped item-level dissociation summaries.",
     "code/experiments/41_item_level_dissociation.py", CUR),
    (r"^item_level_dissociation\.png$",
     "J2 item-level dissociation scatter/plot.",
     "code/experiments/41_item_level_dissociation.py", CUR),
    (r"^CHECKPOINT_DISSECTION\.md$",
     "Checkpoint dissection writeup after digit-token fix; three-family SFT drop.",
     "hand-written", CUR),
    (r"^SIGN_FLIP_RECONCILIATION\.md$",
     "Reconciles sign-flip language across factorial / ladder / checkpoint analyses.",
     "hand-written", CUR),
    (r"^ROSTER_70B_FEASIBILITY\.md$",
     "Whether adding a 70B open model is feasible and whether it breaks open/closed×scale confounds.",
     "hand-written", CUR),
    (r"^REVIEW_CHECKS\.md$",
     "Reviewer checks C1/C5/C6/C7 summary.",
     "code/experiments/35_review_checks.py", CUR),
    (r"^check_c.*\.csv$",
     "Tabular output for one reviewer check (C1/C5/C6/C7).",
     "code/experiments/35_review_checks.py", CUR),
    (r"^HUMAN_ANCHOR_CHOICES\.md$",
     "Which human developmental anchors are plotted and why.",
     "code/34_human_only_ladder.py", NA),
    (r"^human_only_developmental_ladder\.png$",
     "Human-only developmental ladder (no models) for anchor comparison.",
     "code/34_human_only_ladder.py", NA),
    ...
    (r"^gap_over_surface_dissociation\.png$",
     "Peak probe − surface gap by pooling using span-matched TF-IDF baselines.",
     "code/experiments/33_gap_dissociation_figure.py", CUR),
