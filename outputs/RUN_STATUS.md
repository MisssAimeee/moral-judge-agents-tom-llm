...
Updated 2026-07-28 (22:40) after jobs `19030249` (checkpoint factorial rescore), `19027795`
(v3 probe-side refresh) and `19094832`/`19099255` (W3 causal steering + M1/M2/M3) completed,
and W4/W7/W8 started.
...
| Roadmap #7 — reasoning dose–response | **running** | local PIDs via `submit_closed_reasoning.sh` (2026-07-28); sampling n=20, 4-template cut | closed roster in `CLOSED_MODEL_SELECTION.md`; DeepSeek skipped (no key) |
| Closed behavioral rescore (v3) | **running** | same job as #7 | sampling-only; scoring parity bridge via `scoring_parity.csv` |
| Closed BigToM generative (frontier) | **running** | same submit; Anthropic temperature fix applied mid-flight | ToMi dropped |
| W4 — Prompt curriculum | **running** | **19130876** (GPU, l40s, preemptable) | v3; 5 cumulative levels × 6 engaged open models, 7-template basis; both readings pre-registered in `W4_PRESPEC.md` |
...
| W7 — Non-moral selectivity control (demoted to appendix) | **running** | **19131423** (CPU) | Bruneau 2011 appendix stimuli, parsed locally: 144 items / 72 matched pairs |
| W8 — Paper assembly | **in progress** | — (local) | limitations table, provenance appendix, reproducibility manifest in `outputs/paper/` |
